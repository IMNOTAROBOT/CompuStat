...
// [[Rcpp::export]]
List MHBayes(int nsim, NumericVector theta0, Function objdens, Function proposal, NumericMatrix data,NumericMatrix y){
  ...
  for (int i=1; i<nsim; i++){
    ...
    while (!accept && trials<maxtrials){
      X = as<NumericVector>(proposal(theta(i-1,_),data));
      logU = log(R::runif(0,1));
      // the minus is since we used LOGS!!!!!
      if(logU <= as<double>(objdens(data, X, y))-as<double>(objdens(data, theta(i-1,_),y))) { 
        accept = true;
        theta(i,_) = X;
      } 
      trials++;
